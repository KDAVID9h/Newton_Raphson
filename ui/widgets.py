...
class UIWidgets:
    ...
    def on_calculate_click(self, e):
        ...
        initial_guess = [guess_control.controls[0].value for guess_control in self.initial_guess_container.controls 
                        if guess_control.controls[0].value.strip() != ""]
        ...
        equations_sym = [sympify(eq, locals=dict(zip(symbols_list, symbols_list))) for eq in equations]
